...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/02 20:08:16 by nozkara           #+#    #+#             */
/*   Updated: 2024/12/02 20:13:10 by nozkara          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/minishell.h"

static int	open_append_file(t_jobs *jobs, t_job *job, char *filepath,
		int flags[5])
{
	int	fd;
...
static int	check_file_permissions(t_jobs *jobs, t_job *job, char *filepath,
		int fd)
{
...
static int	open_input_file(t_jobs *jobs, t_job *job, char *filepath,
		int flags[5])
{
	int	fd;
	...
	if (check_file_permissions(jobs, job, filepath, fd) == -1)
		return (-1);
...
int	handle_redir(t_jobs *jobs, t_job *job, int flags[5])
{
	...
	if (job->redir->out_f && verify_fd_match(job->redir->out_f[flags[0]],
			filepath, ft_strlen(filepath)) != -1)
		fd = open_output_file(jobs, job, job->redir->out_f[flags[0]++], flags);
	else if (job->redir->app_f && verify_fd_match(job->redir->app_f[flags[1]],
			filepath, ft_strlen(filepath)) != -1)
		fd = open_append_file(jobs, job, job->redir->app_f[flags[1]++], flags);
	else if (job->redir->in_f && verify_fd_match(job->redir->in_f[flags[2]],
			filepath, ft_strlen(filepath)) != -1)
